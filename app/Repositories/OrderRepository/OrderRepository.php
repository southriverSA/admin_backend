...
class OrderRepository extends CoreRepository implements OrderRepoInterface
{
	...
	public function ordersPaginate(array $filter = [], bool $isUser = false): Paginator
	{
		...
		return $order
			...
			->when(
				!$isUser,
				fn($q) => $q
					->when(data_get($filter, 'status') !== Transaction::STATUS_CANCELED || !isset($filter['waiting_order']), function (Builder $q) {
						$q->whereHas(
							'transactions',
							fn($q) => $q
								->where('status', Transaction::STATUS_PAID)
								->orWhereHas('paymentSystem', fn($q) => $q->where('tag', 'cash'))
						)->orWhereDoesntHave('transactions');
					})
			)
			->when(isset($filter['waiting_order']), function ($q) {
	...
	public function orderStocksCalculate(array $filter): array
	{
		...
		$totalPrice = max($totalPrice + $deliveryFee + $shopTax + $serviceFee, 0);
	 ...
	 * @return Order|null
	 */
	public function orderById(int $id, ?int $shopId = null, ?int $userId = null, ?string $phone = null): ?Order
	{
		$locale = data_get(Language::languagesList()->where('default', 1)->first(), 'locale');
